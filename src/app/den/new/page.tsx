...
function NewItemForm({ categoryId }: { categoryId: string }) {
  ...
  // Ingredients (soap, aftershave, balm, preshave, edpedt)
  ...
  const processPhoto = (file: File) => {
    ...
    if (!supported.includes(file.type)) {
      setError('Unsupported photo format. Please use JPEG, PNG, or WEBP.');
      return;
    }
    ...
    reader.onload = () => {
      ...
      img.onerror = () => setError('Could not read photo.');
  ...
  const handleSubmit = async () => {
    ...
    if ((isSoap || isAftershave || isBalm || isPreshave || isEdpEdt) && ingredients.trim()) data.ingredients = ingredients.trim();
      ...
      <div className="space-y-6">
        ...
        <div className="bg-[#1e1e1e] rounded-2xl border border-white/5 p-6 space-y-4">
          ...
          <div>
            ...
            <div
              className={`relative w-28 aspect-square bg-[#242424] rounded-xl overflow-hidden border cursor-pointer group transition-colors ${isDragging ? "border-[#c9a050]/60 bg-[#c9a050]/5" : "border-white/10"}`}
        ...
        {(isSoap || isAftershave) && (
          <div className="bg-[#1e1e1e] rounded-2xl border border-white/5 p-6 space-y-5">
            ...
            <div>
              <label className="block text-xs text-gray-500 uppercase tracking-wider mb-1.5">Ingredients <span className="normal-case text-gray-600">(optional)</span></label>
