...
function isKnownBrand(
  category: string,
  brand: string,
  razorBrands: RazorBrand[],
  categoryBrands: CategoryBrandsData
): boolean {
  if (category === "razor") return razorBrands.some((b) => b.name === brand);
  if (category === "soap") return categoryBrands.soap.some((b) => b.name === brand);
  if (category === "aftershave") return categoryBrands.aftershave.some((b) => b.name === brand);
  if (category === "blade") return categoryBrands.blade.some((b) => b.name === brand);
  if (category === "brush") return categoryBrands.brush.includes(brand);
  return false;
}
...
function isKnownModel(
  ...
  if (category === "razor") {
    const rb = razorBrands.find((b) => b.name === brand);
    return rb?.models.includes(model) ?? false;
  }
  const cats =
    category === "soap" ? categoryBrands.soap :
    category === "aftershave" ? categoryBrands.aftershave :
    category === "blade" ? categoryBrands.blade : [];
  return (cats as CategoryBrand[]).find((b) => b.name === brand)?.items.includes(model) ?? false;
}

// ---------------------------------------------------------------------------
// Shared form component used for both "Add alert" and inline "Edit alert"
// ---------------------------------------------------------------------------
function AlertForm({
  initialValues,
  razorBrands,
  ...
  initialValues?: Partial<WatchlistAlert>;
  razorBrands: RazorBrand[];
  categoryBrands: CategoryBrandsData;
  onSave: (payload: AlertPayload) => Promise<void>;
  onCancel: () => void;
  title?: string;
  submitLabel?: string;
}) {
  const initCategory = initialValues?.category ?? "razor";
  const initBrandRaw = initialValues?.brand ?? "";
  const brandIsKnown = initBrandRaw
    ? isKnownBrand(initCategory, initBrandRaw, razorBrands, categoryBrands)
    : true;
  const initBrand = initBrandRaw ? (brandIsKnown ? initBrandRaw : CUSTOM_BRAND_KEY) : "";
  const initCustomBrand = brandIsKnown ? "" : initBrandRaw;

  const initModelRaw = initialValues?.model ?? "";
  const modelIsKnown = initModelRaw && initBrandRaw && brandIsKnown
    ? isKnownModel(initCategory, initBrandRaw, initModelRaw, razorBrands, categoryBrands)
    : true;
  const initModel = initModelRaw ? (modelIsKnown ? initModelRaw : CUSTOM_BRAND_KEY) : "";
  const initCustomModel = modelIsKnown ? "" : initModelRaw;

  const [formCategory, setFormCategory] = useState(initCategory);
  const [formBrand, setFormBrand] = useState(initBrand);
  const [formModel, setFormModel] = useState(initModel);
  const [formMaterial, setFormMaterial] = useState(initialValues?.material ?? "");
  const [formKeyword, setFormKeyword] = useState(initialValues?.keyword ?? "");
  const [formMaxPrice, setFormMaxPrice] = useState(
    initialValues?.maxPrice != null ? String(initialValues.maxPrice) : ""
  );
  const [customBrandInput, setCustomBrandInput] = useState(initCustomBrand);
  const [customModelInput, setCustomModelInput] = useState(initCustomModel);
  ...
  return (
    <form onSubmit={handleSubmit} className="px-5 py-4 border-b border-white/5 space-y-3">
      {title && <p className="text-xs text-gray-400 font-medium uppercase tracking-wide">{title}</p>}
...
export default function WatchlistSection() {
  ...
  const handleAddAlert = async (payload: AlertPayload) => {
    await api.post("/api/bst/alerts", payload);
    await loadAlerts();
    setShowForm(false);
  };

  const handleEditAlert = async (id: string, payload: AlertPayload) => {
    const updated = await api.patch<{ alert: WatchlistAlert }>(`/api/bst/alerts/${id}`, payload);
    setAlerts((prev) => prev.map((a) => (a.id === id ? { ...updated.alert, matchCount: a.matchCount } : a)));
    setEditingId(null);
  };
            ...
            <li key={alert.id}>
              ...
              <div className="flex items-center justify-between px-5 py-3 gap-3">
                ...
                <div className="flex items-center gap-1 shrink-0">
                  <button
                    onClick={() => setEditingId(editingId === alert.id ? null : alert.id)}
              ...
              {editingId === alert.id && (
                <div className="border-t border-white/5 bg-[#1e1e1e]">
                  <AlertForm
                    ...
                    razorBrands={razorBrands}
                    categoryBrands={categoryBrands}
                    onSave={(payload) => handleEditAlert(alert.id, payload)}
